...
import {Loader} from "../Assets"; // Ensure this points to the actual video file
...
const LoadingSpinner = () => {
  ...
  useEffect(() => {
    const videos = [carVideoRef.current, carVideoRef.current];
    videos.forEach((video) => {
      if (video) {
        video
          .play()
          .catch((error) => console.error("Video playback failed:", error));
        video.loop = true; // Ensures videos loop
      }
    });
  }, []);
